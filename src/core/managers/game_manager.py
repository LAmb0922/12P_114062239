...
class GameManager:
    ...
    def __init__(self, maps: dict[str, Map], start_map: str, 
                player: Player | None,
                enemy_trainers: dict[str, list[EnemyTrainer]], shop: dict[str,list[Shop]],
                bag: Bag | None = None ):###
    ...
    @property
    def current_shop(self) ->list[Shop]:
        return self.shop[self.current_map_key]
    @property
    ...
    def check_collision(self, rect: pg.Rect) -> bool:
        ...
        for entity in self.enemy_trainers[self.current_map_key]:
            if rect.colliderect(entity.animation.rect):
                return True
        for shop in self.shop[self.current_map_key]:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        for key, m in self.maps.items():
            ...
            block["shop"]=[t.to_dict() for t in self.shop.get(key ,[])]
    ...
    def from_dict(cls, data: dict[str, object]) -> "GameManager":
        ...
        for m in data["map"]:
            raw_data=m["shop"]
            gm.shop[m["path"]]=[Shop.from_dict(t,gm) for t in raw_data]
