...
class ChatOverlay(UIComponent):
    ...
    def __init__(
        ...
        *,
        ...
        # TODO
        try:
            self._font_msg = pg.font.Font(font_path, 12)
            self._font_input = pg.font.Font(font_path, 14)
        except Exception:
            self._font_msg = pg.font.SysFont("arial", 12)
    ...
    def _handle_typing(self) -> None:
        ...
        if input_manager.key_pressed(pg.K_RETURN) or input_manager.key_pressed(pg.K_KP_ENTER):
            ...
            if txt and self._send_callback:
                ok = False
                try:
                    ok = self._send_callback(txt) #<- over here we need to send chat message, what function should we call?
                except Exception:
                    ok = False
                if ok:
                    self._input_text = ""
    ...
    def draw(self, screen: pg.Surface) -> None:
        ...
        text_surf = self._font_input.render(txt, True, (255, 255, 255)) #<- over here we need to RENDER the text, what function should we call?
